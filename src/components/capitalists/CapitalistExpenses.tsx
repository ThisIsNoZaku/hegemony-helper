...
export default function CapitalistExpenses({revenue, companies, laws} : {revenue: number, companies:CompanyInstance[], laws:Record<LawId, LawLevel>}) {
    ...
    const estimatedWages = {
        mc: _.sum(operationalCompanies.filter(c => c.workers === "mc").map(c => c.wages[c.wageLevel] || 0)),
        wc: _.sum(operationalCompanies.filter(c => c.workers === "wc").map(c => c.wages[c.wageLevel] || 0))
    }
